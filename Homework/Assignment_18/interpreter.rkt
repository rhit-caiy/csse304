...
(define apply-while
  (lambda (test body env k)
    (if (eval-exp test env k)
        (apply-while-hlp test body env (map (lambda (a) (eval-exp a env k)) body) k)
        (void))))
